...
export async function bookAppointment(driver) {
  ...
  try {
    ...
    await driver
      .findElement(
        By.xpath(
          `//button[@aria-label='${format(addDays(date, 1), "EEEE, LLLL do, y")}']`,
        ),
      )
      .click();
    console.log(`Selected day: ${format(addDays(date, 1), "d")}`);
    await sleep(2000);
    ...
    await driver
      .findElement(
        By.xpath(
          `//button[@aria-label='${format(addDays(date, 2), "EEEE, LLLL do, y")}']`,
        ),
      )
      .click();
    console.log(`Selected day: ${format(addDays(date, 2), "d")}`);
    await sleep(2000);
