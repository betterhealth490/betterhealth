...
export async function getHealthHabit(
  input: GetHealthHabitsInput,
): Promise<GetHealthHabitsResult> {
  const result = await db
    .select()
    .from(healthHabits)
    .where(eq(healthHabits.habitId, input.habitId));
...
export async function listHealthHabit(
  input: ListHealthHabitsInput,
): Promise<ListHealthHabitsResult> {
  const result = await db
    .select()
    .from(healthHabits)
    .where(eq(healthHabits.patientId, input.userId));
...
export async function updateHealthHabit(
  input: UpdateHealthHabitsInput,
): Promise<UpdateHealthHabitsResult> {
  const result = db
    .update(healthHabits)
    .set({
      date: input.date,
...
export async function createHealthHabit(
  input: CreateHealthHabitsInput,
): Promise<CreateHealthHabitsResult> {
  const result = await db
    .insert(healthHabits)
    .values({
      date: input.date,
