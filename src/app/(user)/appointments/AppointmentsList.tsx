...
export default function AppointmentsList({
  ...
  const [filter, setFilter] = useState<string>("all"); // Filter by therapist name or 'all'
  ...
  const fetchAppointments = async () => {
    try {
      ...
      setFilteredAppointments(result); // Initialize filtered appointments
    } catch (error) {
  ...
  useEffect(() => {
    fetchAppointments();
  }, [patientId, refresh]); // Refetch when patientId or refresh changes
