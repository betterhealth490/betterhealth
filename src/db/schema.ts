...
// Audit Tables
export const healthHabitsAudit = createTable(
    "health_habits_audit",
    {
        auditId: serial("audit_id").primaryKey(),
        habitId: integer("habit_id").notNull().references(() => healthHabits.habitId),
        userId: integer("user_id").notNull().references(() => users.userId),
...
export const surveysAudit = createTable(
    "surveys_audit",
    {
        auditId: serial("audit_id").primaryKey(),
        surveyId: integer("survey_id").notNull().references(() => surveys.surveyId),
        userId: integer("user_id").notNull().references(() => users.userId),
        changeType: changeTypeEnum("change_type").notNull(),
        changedAt: timestamp("changed_at").default(sql`CURRENT_TIMESTAMP`).notNull(),
        previousData: json("previous_data"),
...
export const billingAudit = createTable(
    "billing_audit",
    {
        auditId: serial("audit_id").primaryKey(),
        billId: integer("bill_id").notNull().references(() => billing.billId),
        userId: integer("user_id").notNull().references(() => users.userId),
        changeType: changeTypeEnum("change_type").notNull(),
        changedAt: timestamp("changed_at").default(sql`CURRENT_TIMESTAMP`).notNull(),
        previousData: json("previous_data"),
